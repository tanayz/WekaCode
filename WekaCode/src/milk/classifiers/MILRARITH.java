 ...
 */
public class MILRARITH extends MIClassifier implements OptionHandler {
	...
	for (int j = 0; j < nR; j++) {
	    ...
	    //xSD[j] = Math.sqrt(xSD[j]/totIns-xMean[j]*xMean[j]*totIns/(totIns-1.0));  
	...
	//opt.setMaxIteration(200*x.length);
	...
	for(int k=idx.length-1; k>=0; k--)
	    System.out.println(m_Attributes.attribute(idx[k]+1).name()+"\t"+(fs[idx[k]]*100/max));
   ...
   */
    public static void main(String [] argv) {
      try {
	  System.out.println(MIEvaluation.evaluateModel(new MILRARITH(), argv));
      } catch (Exception e) {
	  e.printStackTrace();
