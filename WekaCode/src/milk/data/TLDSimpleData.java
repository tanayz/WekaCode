...
/**
 * Generate n-dimensional 2-class MI data using two level distributions, Simpler version
 * prior = 0.5
 */
public class TLDSimpleData{   
	...
	try{
	    ...
	    double[][] sgm = new double[2][numAtts], 
		w = new double[2][numAtts], m = new double[2][numAtts];
	    int SIZE = 0;
	    
	    for(int i=0; i<2; i++){
		for(int j=0; j<numAtts; j++){
		    sgm[i][j] = Double.parseDouble(args[index++]);
	    ...
	    pw.println("%Positive:");
	    for(int j=0; j<numAtts; j++)
		pw.println("%Att "+j+": sigma="+sgm[0][j]+", w="+w[0][j]+", m="+m[0][j]);
	    pw.println("%Negative:");
	    for(int j=0; j<numAtts; j++)
		pw.println("%Att "+j+": sigma="+sgm[1][j]+", w="+w[1][j]+", m="+m[1][j]);
		...
		for(int y=0; y < numAtts; y++){			
		    double mu = ran.nextGaussian()*Math.sqrt(w[c][y])+m[c][y]; 
		    param[x][y][0] = mu;
		    param[x][y][1] = sgm[c][y];
		}
	    }
		...
		int size = 20;//1+ran.nextInt(20); //size		
