 ...
 */
public class MIRunPanel extends JPanel implements ActionListener {
  ...
  class ExperimentRunner extends Thread implements Serializable {
    ...
    MIExperiment m_ExpCopy;
    
    public ExperimentRunner(final MIExperiment exp) throws Exception {
	...
	System.err.println("Running experiment: " + exp.toString());
      ...
      m_ExpCopy = (MIExperiment) so.getObject();
      System.err.println("Made experiment copy");
    }

    public void abortExperiment() {
      if (m_ExpCopy instanceof MIRemoteExperiment) {
	((MIRemoteExperiment)m_ExpCopy).abortExperiment();
	...
	if (!(m_ExpCopy instanceof MIRemoteExperiment)) {
	  ...
	  ((MIRemoteExperiment)m_ExpCopy).runExperiment();
	}
      } catch (Exception ex) {
	ex.printStackTrace();
   ...
   */
  public MIRunPanel(MIExperiment exp) {
  ...
  public static void main(String [] args) {
	...
	if (to instanceof MIRemoteExperiment) {
	  exp = (MIRemoteExperiment)to;
	} else {
	  exp = (MIExperiment)to;
	}
	oi.close();
	...
	exp = new MIExperiment();
      ...
      final MIRunPanel sp = new MIRunPanel(exp);
