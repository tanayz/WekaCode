 ...
 */
public class MISetupPanel extends JPanel {

  /** The experiment being configured */
  protected MIExperiment m_Exp;
  ...
  protected FileFilter m_ExpFilter = 
    new ExtensionFileFilter(MIExperiment.FILE_EXTENSION, 
  ...
  protected MIGeneratorPropertyIteratorPanel m_GeneratorPropertyPanel
    = new MIGeneratorPropertyIteratorPanel();

  /** The panel for configuring run numbers */
  protected MIRunNumberPanel m_RunNumberPanel = new MIRunNumberPanel();

  /** The panel for enabling a distributed experiment */
  protected MIDistributeExperimentPanel m_DistributeExperimentPanel = 
    new MIDistributeExperimentPanel();

  /** The panel for configuring selected datasets */
  protected MIDatasetListPanel m_DatasetListPanel = new MIDatasetListPanel();
  ...
  static {
    ...
    java.beans.PropertyEditorManager
      .registerEditor(java.io.File.class,
		      FileEditor.class);
    java.beans.PropertyEditorManager
      .registerEditor(milk.experiment.MIResultListener.class,
		      GenericObjectEditor.class);
    java.beans.PropertyEditorManager
      .registerEditor(milk.experiment.MIResultProducer.class,
		      GenericObjectEditor.class);
    java.beans.PropertyEditorManager
      .registerEditor(milk.experiment.MISplitEvaluator.class,
		      GenericObjectEditor.class);
    java.beans.PropertyEditorManager
      .registerEditor(milk.classifiers.MIClassifier.class,
		      GenericObjectEditor.class);
    java.beans.PropertyEditorManager
   ...
   */
  public MISetupPanel(MIExperiment exp) {
  ...
  public MISetupPanel() {
    ...
    m_DistributeExperimentPanel.
	  ...
	  public void actionPerformed(ActionEvent e) {
	    if (m_DistributeExperimentPanel.distributedExperimentSelected()) {
	      if (!(m_Exp instanceof MIRemoteExperiment)) {
		try {
		  MIRemoteExperiment re = new MIRemoteExperiment(m_Exp);
		  setExperiment(re);
		} catch (Exception ex) {
		  ex.printStackTrace();
		}
	      ...
	      if (m_Exp instanceof MIRemoteExperiment) {
		setExperiment(((MIRemoteExperiment)m_Exp).getBaseExperiment());
	      }
	    }
	  }
	});	      
    ...
    m_NewBut.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
	setExperiment(new MIExperiment());
      }
    });
    ...
    m_RPEditor.setClassType(MIResultProducer.class);
    m_RPEditor.setEnabled(false);
    m_RPEditor.addPropertyChangeListener(new PropertyChangeListener() {
      public void propertyChange(PropertyChangeEvent e) {
	m_Exp.setResultProducer((MIResultProducer) m_RPEditor.getValue());
    ...
    m_RLEditor.setClassType(MIResultListener.class);
    m_RLEditor.setEnabled(false);
    m_RLEditor.addPropertyChangeListener(new PropertyChangeListener() {
      public void propertyChange(PropertyChangeEvent e) {
	m_Exp.setResultListener((MIResultListener) m_RLEditor.getValue());
   ...
   * @param exp a value of type 'Experiment'
   */
  public void setExperiment(MIExperiment exp) {
   ...
   */
  public MIExperiment getExperiment() {
  ...
  private void openExperiment() {
    ...
    if (!expFile.getName().toLowerCase().endsWith(MIExperiment.FILE_EXTENSION)) {
      expFile = new File(expFile.getParent(), expFile.getName()
                         + MIExperiment.FILE_EXTENSION);
    }
    try {
      FileInputStream fi = new FileInputStream(expFile);
      ObjectInputStream oi = new ObjectInputStream(
			     new BufferedInputStream(fi));
      MIExperiment exp = (MIExperiment)oi.readObject();
      oi.close();
      setExperiment(exp);
  ...
  public static void main(String [] args) {
	...
	exp = new MIExperiment();
      ...
      final MISetupPanel sp = new MISetupPanel();
