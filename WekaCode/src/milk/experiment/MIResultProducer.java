 ...
 *
 * Possible implementations of MIResultProducer: <br>
 * <ul>
...
public interface MIResultProducer extends Serializable {
   ...
   */
  void setInstances(Exemplars instances);
   ...
   */
  void setResultListener(MIResultListener listener);
  ...
  /**
   * Prepare to generate results. The MIResultProducer should call
   ...
   * will be sent. The MIResultProducer should call
   * preProcess(this) on the ResultListener it is to send results to.
   *
   ...
   * producer, sufficient for distinguishing a MIResultProducer
   * instance from another with different settings (ignoring
   * those settings set through this interface). For example,
   * a cross-validation MIResultProducer may have a setting for the
   * number of folds. For a given state, the results produced should
   * be compatible. Typically if a MIResultProducer is an OptionHandler,
   * this string will represent those command line arguments required
   * to set the MIResultProducer to that state.
   *
   * @return the description of the MIResultProducer state, or null
