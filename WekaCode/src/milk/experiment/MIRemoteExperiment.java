 ...
 *
 * The machine where a MIRemoteExperiment is started must also have a copy
 * of DatabaseUtils.props listing the URL to the machine where the 
 * database server is running (RmiJdbc + InstantDB). <p>
 *
 * Here is an example of starting a MIRemoteExperiment: <p>
 *
 * <pre>
 *
 * java -Djava.rmi.server.codebase=file:/path to weka classes/ \
 * weka.experiment.MIRemoteExperiment -L 1 -U 10 \
 ...
 */
public class MIRemoteExperiment extends MIExperiment {
...
//    protected static final int FINISHED=3;
  ...
  /** The base experiment to split up into sub experiments for remote
      execution */
  private MIExperiment m_baseExperiment = null;

  /** The sub experiments */
  protected MIExperiment [] m_subExperiments;
   ...
   */
  public MIRemoteExperiment(MIExperiment base) throws Exception {
    setBaseExperiment(base);
  }

  /**
   * Add an object to the list of those interested in recieving update
   * information from the MIRemoteExperiment
   * @param r a listener
   */
   ...
   */
  public MIExperiment getBaseExperiment() {
   ...
   */
  public void setBaseExperiment(MIExperiment base) throws Exception {
    if (base == null) {
   ...
   */
  public void setResultListener(MIResultListener newResultListener) {
   ...
   * experiment.
   */
  public void setResultProducer(MIResultProducer newResultProducer) {
  ...
  public static void main(String[] args) {
    ...
    try {
      MIRemoteExperiment exp = null;
      MIExperiment base = null;
	...
	if (tmp instanceof MIRemoteExperiment) {
	  exp = (MIRemoteExperiment)tmp;
	} else {
	  base = (MIExperiment)tmp;
	}
	oi.close();
      }
      if (base != null) {
	exp = new MIRemoteExperiment(base);
      }
